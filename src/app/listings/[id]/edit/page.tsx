...
type ExistingMedia = ListingMediaItem;
type NewMedia = { file: File; previewUrl: string; type: ListingMediaType };
            ...
            <div key={num} className="flex flex-1 items-center last:flex-none">
              <div className="flex flex-col items-center gap-1.5">
                <div className={`flex h-9 w-9 items-center justify-center rounded-full border-2 transition-all duration-300 ${
                  isDone ? "border-primary bg-primary text-white" : isActive ? "border-primary bg-white text-primary shadow-md shadow-primary/20" : "border-border bg-background text-muted"
                }`}>
                  {isDone ? (
...
export default function EditListingPage() {
  ...
  const { user, listings, updateListing } = useAuth();
  const fileInputRef = useRef<HTMLInputElement>(null);

  const listing = listings.find((l) => l.id === params.id);
  const isVenueOwner = user?.role === "venue" && listing?.venueId === user?.accountId;
  ...
  useEffect(() => {
    ...
    setEquipment(listing.equipmentIncluded ?? []);
    setHouseRules(listing.houseRules ?? []);
    setExistingMedia(listing.media ?? []);
    setListingMode(listing.listingMode ?? "one_off");
    setSlotCapacity(listing.slotCapacity ?? 1);
    const slots = DAY_NAMES.map((_, day) => {
      const match = listing.availability?.find((a) => a.day === day);
      return match ? { day, enabled: true, start: match.start, end: match.end } : { day, enabled: false, start: "09:00", end: "17:00" };
    });
    setAvailability(slots);
    setInitialized(true);
  }, [listing, initialized]);

  // Load venue photo library
  useEffect(() => {
    if (!user?.accountId) return;
    fetch(`/api/venue-profile?venueId=${user.accountId}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => { if (data?.photos?.length) setVenuePhotos(data.photos); })
      .catch(() => {});
  }, [user?.accountId]);
  ...
  const toggleLibraryPhoto = (url: string) => {
    setExistingMedia((prev) => {
      const exists = prev.some((m) => m.url === url);
      if (exists) return prev.filter((m) => m.url !== url);
      if (prev.length + newMediaFiles.length >= MAX_MEDIA) return prev;
      return [...prev, { url, type: "image" as ListingMediaType }];
    });
  };
  ...
  const addMediaFiles = (files: FileList | null) => {
    ...
    for (let i = 0; i < files.length && totalMedia + toAdd.length < MAX_MEDIA; i++) {
      const file = files[i];
      if (!file.type.startsWith("image/") && !file.type.startsWith("video/")) continue;
      toAdd.push({ file, previewUrl: URL.createObjectURL(file), type: fileToType(file) });
    }
    if (toAdd.length) setNewMediaFiles((prev) => [...prev, ...toAdd]);
  };

  const removeExistingMedia = (url: string) => setExistingMedia((prev) => prev.filter((m) => m.url !== url));
  const removeNewMedia = (previewUrl: string) => {
    setNewMediaFiles((prev) => {
      const entry = prev.find((e) => e.previewUrl === previewUrl);
      if (entry) URL.revokeObjectURL(entry.previewUrl);
      return prev.filter((e) => e.previewUrl !== previewUrl);
    });
  };
  ...
  const handleSave = async () => {
    if (!listing || !title || !description || !price || !location) return;
    ...
    let uploadedNew: ListingMediaItem[] = [];
    try {
      for (const entry of newMediaFiles) {
        const formData = new FormData();
        formData.append("file", entry.file);
        const res = await fetch("/api/upload", { method: "POST", body: formData });
        if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error ?? "Upload failed");
        const data = (await res.json()) as { url: string };
        uploadedNew.push({ url: data.url, type: entry.type });
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-background"><Navbar />
        <div className="flex min-h-[60vh] items-center justify-center">
          <p className="text-lg font-medium text-foreground">Only venue accounts can edit listings.</p>
        </div>
      </div>
    );
  }
  ...
  if (!isVenueOwner) {
    return (
      <div className="min-h-screen bg-background"><Navbar />
        <div className="flex min-h-[60vh] items-center justify-center">
          <p className="text-lg font-medium text-foreground">You can only edit your own listings.</p>
        </div>
      </div>
    );
  }
  ...
  const staticMapUrl = latitude != null && longitude != null && process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY
    ...
    : null;
  const allMedia = [...existingMedia, ...newMediaFiles.map((m) => ({ url: m.previewUrl, type: m.type }))];
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted">Update details for &ldquo;{listing.title}&rdquo;</p>
        </div>
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="mb-3 text-xs text-muted">One-off listings are removed once a booking is completed. Recurring listings stay online permanently.</p>
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-foreground">Description <span className="text-danger">*</span></label>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <input type="number" value={price} onChange={(e) => setPrice(e.target.value)}
                  placeholder={priceType === "daily" ? "150" : "800"} required min="1"
                  className="w-full rounded-lg border border-border bg-background px-4 py-3 text-sm text-foreground placeholder:text-muted focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
              </div>
              <div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div>
              <label className="mb-1 block text-sm font-semibold text-foreground">Address <span className="text-danger">*</span></label>
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div>
              ...
              <p className="mb-3 text-xs text-muted">Up to {MAX_MEDIA} images or videos. First image is the cover.</p>
                ...
                <div className="mb-3 rounded-xl border border-primary/20 bg-primary/5 p-4">
                  ...
                  {showLibrary && (
                    <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-4">
                      {venuePhotos.map((url) => {
                        const selected = existingMedia.some((m) => m.url === url);
                        return (
              ...
              <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
                ...
                className={`flex min-h-[100px] cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed bg-background transition-colors ${dragOver ? "border-primary bg-primary-light/20" : "border-border hover:border-primary/50"}`}>
                <svg className="h-8 w-8 text-muted/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0022.5 18.75V5.25A2.25 2.25 0 0020.25 3H3.75A2.25 2.25 0 001.5 5.25v13.5A2.25 2.25 0 003.75 21z" />
                </svg>
                <p className="mt-1 text-sm text-muted">Click or drag to add more</p>
              </div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                  {existingMedia.map((entry) => (
                    <div key={entry.url} className="group relative aspect-video overflow-hidden rounded-lg border border-border bg-muted/30">
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); removeExistingMedia(entry.url); }}
                  ...
                  {newMediaFiles.map((entry) => (
                    <div key={entry.previewUrl} className="group relative aspect-video overflow-hidden rounded-lg border-2 border-dashed border-primary/40 bg-muted/30">
                      {entry.type === "video"
                        ? <video src={entry.previewUrl} className="h-full w-full object-cover" muted playsInline preload="metadata" />
                        : <img src={entry.previewUrl} alt="" className="h-full w-full object-cover" />
                      }
                      <span className="absolute left-1.5 top-1.5 rounded bg-primary/80 px-1.5 py-0.5 text-xs text-white">New</span>
                      <button type="button" onClick={(e) => { e.stopPropagation(); removeNewMedia(entry.previewUrl); }}
                        className="absolute right-1.5 top-1.5 flex h-6 w-6 items-center justify-center rounded-full bg-danger text-white opacity-0 transition group-hover:opacity-100 cursor-pointer"
                        aria-label="Remove">
                        <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={addEquipment}
                  className="shrink-0 rounded-lg bg-primary-light px-4 py-3 text-sm font-medium text-primary hover:bg-primary hover:text-white transition-colors cursor-pointer">Add</button>
              </div>
              {equipment.length > 0 && (
            ...
            <StepFooter step={step} onBack={goBack} onNext={goNext} canNext nextLabel="Review &amp; Save" />
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-6">
            <p className="text-sm text-muted">Review your changes before saving.</p>

            <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
              {allMedia.length > 0 ? (
                <div className="relative h-52 w-full overflow-hidden bg-muted/20">
                  {allMedia[0].type === "video"
                    ? <video src={allMedia[0].url} className="h-full w-full object-cover" muted playsInline preload="metadata" />
                    : <img src={allMedia[0].url} alt="" className="h-full w-full object-cover" />
                  }
                  {allMedia.length > 1 && (
                    <div className="absolute bottom-3 right-3 rounded-lg bg-black/60 px-2 py-1 text-xs text-white backdrop-blur-sm">+{allMedia.length - 1} more</div>
                  )}
                  {isRecurring && (
                ...
                <div className="flex h-40 items-center justify-center bg-muted/10 text-muted text-sm">No photos</div>
              )}
              ...
              <div className="p-5 space-y-4">
                ...
                {enabledDays.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {enabledDays.map((s) => (
                      <span key={s.day} className="rounded-md bg-primary/10 px-2 py-0.5 text-[11px] font-semibold text-primary">{DAY_NAMES[s.day]}</span>
                    ))}
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { d: "M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0022.5 18.75V5.25A2.25 2.25 0 0020.25 3H3.75A2.25 2.25 0 001.5 5.25v13.5A2.25 2.25 0 003.75 21z", label: `${allMedia.length} photo${allMedia.length !== 1 ? "s" : ""}` },
                    { d: "M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5", label: `${enabledDays.length} day${enabledDays.length !== 1 ? "s" : ""} available` },
            ...
            <StepFooter step={step} onBack={goBack} onNext={handleSave} canNext={!isSubmitting}
              nextLabel={isSubmitting ? "Saving…" : "Save Changes"} isPublish />
          </div>
        )}
      </div>
    </div>
...
function StepFooter({
  step, onBack, onNext, canNext, nextLabel, isPublish = false,
}: {
  step: number; onBack: () => void; onNext: () => void; canNext: boolean; nextLabel: string; isPublish?: boolean;
}) {
  return (
    <div className="flex items-center justify-between border-t border-border pt-6">
      ...
      <button type="button" onClick={onNext} disabled={!canNext}
        className={`inline-flex items-center gap-2 rounded-xl px-6 py-2.5 text-sm font-semibold text-white shadow-lg transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${
          isPublish ? "bg-success shadow-success/25 hover:bg-green-700" : "bg-primary shadow-primary/25 hover:bg-primary-dark"
        }`}>
