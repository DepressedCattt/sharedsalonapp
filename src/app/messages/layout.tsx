...
export default function MessagesLayout({ children }: { children: React.ReactNode }) {
  ...
  const activeConvId = pathname.startsWith("/messages/")
    ...
    : null;
  const isConversationOpen = !!activeConvId;
  ...
  return (
    <div className="flex flex-col bg-background" style={{ height: "100dvh" }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside
          className={`flex-col border-r border-border bg-card shrink-0 w-full md:w-80 lg:w-96 ${
        ...
        {/* Right panel — hidden on mobile when no conversation is open */}
        <main
              ...
              <button
                ...
                className={`flex w-full items-start gap-3 px-4 py-3.5 text-left transition-colors cursor-pointer border-b border-border/50 last:border-0 ${
                  isActive
                    ? "bg-primary/10"
                    : conv.unreadCount > 0
                    ? "bg-primary/[0.03] hover:bg-muted/10"
                    : "hover:bg-muted/10"
                }`}
              >
                {/* Avatar */}
                <div className="relative mt-0.5 shrink-0">
                  {other.avatarUrl ? (
                    ...
                    <div
                      className={`flex h-11 w-11 items-center justify-center rounded-full text-sm font-bold ${color}`}
                    >
                  ...
                  <div className="mt-1 flex items-end justify-between gap-2">
                    <p
                      className={`text-xs truncate leading-snug ${
                        conv.unreadCount > 0 ? "font-medium text-foreground" : "text-muted"
                      }`}
                    >
                      {conv.lastMessage ? (
                        <>
